...
class ROS_image_occupancy_creator():
	def __init__(self):
		...
		self.pathSub = rospy.Subscriber('/global_planner/planner/plan', Path, self.path_callback)
	...
	def odom_callback(self, ros_odom):
		if self.occupancy_received == True:
			...
			image_to_send = self.image.copy() # without .copy method it writes directly on self.image
			...
			if(len(self.path_points) > 0):
				for i in range(0,len(self.path_points)):
					pointPoseX = int((self.path_points[i].pose.position.x + 10)/(self.resolution))
					pointPoseY = int((self.path_points[i].pose.position.y + 10)/(self.resolution))
					image_to_send[pointPoseY, pointPoseX] = 150
	...
	def path_callback(self, path_message):
		self.path_points = path_message.poses
